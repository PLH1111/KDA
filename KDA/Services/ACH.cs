...
namespace KDA.Services
{
    ...
    public class ACH
    {
        ...
        public static SleepModel GetSleep()
        {
            ...
            {
                SleepModel model = new SleepModel((SleepTimes)bytes[1], (LightingModes)bytes[2]);
                return model;
            }
        }
        ...
        public static KeyMacroModel GetKeyMacro(byte[] KeyIndex)
        {
            ...
            {
                KeyMacroModel model = new KeyMacroModel(bytes[1], (KeyModes)bytes[2], bytes[3]);
                return model;
            }
        }
        ...
        public static KeyColorModel GetKeyColor(byte keyIndex)
        {
            ...
            {
                KeyColorModel model = new KeyColorModel(bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]);
                return model;
            }
        }
        ...
        public static AnimationModel GetAnimation()
        {
            ...
            {
                AnimationModel model = new AnimationModel((AnimationIds)bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6],
                                           (AnimationDisplays)bytes[7], (ColorNum)bytes[8]);
        ...
        public static ProfileModel GetProfile()
        {
            ...
            {
                ProfileModel model = new ProfileModel(bytes[0]);
                return model;
            }
        }
        ...
        public static KeyColorMap GetColorMap()
        {
            ...
            {
                KeyColorMap map = new KeyColorMap(bytes[0]);
                for (byte i = 0; i < 11; i++)
                {
        ...
        public static MarcoMap GetMarcoMap(byte mapNo)
        {
            ...
            {
                MarcoMap map = new MarcoMap(bytes[0])
                {
                    Index = bytes[1]
                };
        ...
        public static BootUpMap GetBootUpMap(byte mapNo)
        {
            ...
            {
                BootUpMap map = new BootUpMap(bytes[0]);
                for (byte i = 0; i < 7; i++)
                {
